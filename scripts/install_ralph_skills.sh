...
# Install the Ralph skills (prd, ralph_runner, ralph_container_runner) into OpenCode, Amp, or Claude skills directories.
# Usage:
...
install_set() {
  local dest="$1"
  mkdir -p "$dest"
  for SKILL_NAME in prd ralph_runner ralph_container_runner; do
