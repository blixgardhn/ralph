...
- Recording state and decisions in progress.md and ralph/context_rules/state.json
...
- Ralph must create and maintain: `ralph/observe.sh`, `ralph/verify.sh`, `progress.md`, `ralph/context_rules/state.json`, and `ralph/state/next_mode.txt`.
...
ralph/context_rules/RULES-dotnet.md and ralph/context_rules/RULES-python.md are authoritative constraints. Apply the relevant language rules; if they conflict with other instructions, Ralph must resolve the conflict explicitly (PLAN or SPEC GAP), never silently.
