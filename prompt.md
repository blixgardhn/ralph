...
5) If the story touches code, run the smallest relevant checks first (targeted tests/lint/typecheck) using project commands; prefer containerized entrypoints if available. When tests or SDKs are needed and Ralph is already inside a container, spawn a fresh, purpose-built test container (smallest practical base image matching the stack) instead of reusing the runner container.
...
14) Applications must be designed to run in Kubernetes: container-friendly commands/configs, env-based settings, stateless processes, health/readiness endpoints when applicable, and no reliance on host-only tooling.
